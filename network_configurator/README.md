...
└── tools/                # Custom tools (optional)
    └── network_tools.py   # Example network tools
```
...
Creates `network_configurator.ncp` containing all agent files and dependencies.
...
```bash
# First deployment
ncp deploy network_configurator.ncp

# Update existing agent
ncp deploy network_configurator.ncp --update
```
...
```bash
ncp playground --agent network_configurator

# Show tool calls and results
ncp playground --agent network_configurator --show-tools
```
...
```bash
ncp remove --agent network_configurator
```
...
4. **Deploy/Update** - `ncp deploy network_configurator.ncp --update`
5. **Test** - `ncp playground --agent network_configurator`
6. **Iterate** - Repeat steps 1-5

### Adding Custom Tools

Create tools in your agent file or in `tools/`. This project includes example network tools:
